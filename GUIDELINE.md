...
| `serviceId` | 직접 연동 시 별도 문의 (dev.prod_api_support@gripcorp.co)<br>쇼핑몰 통합관리 솔루션(서드파티 솔루션)을 통해 연동하는 경우 신청 불필요 |
...
`serviceId`는 판매자 단위가 아닌 **API 연동 서비스(시스템) 단위**로 발급되는 식별자입니다.

### 쇼핑몰 통합관리 솔루션을 통해 연동하는 경우

쇼핑몰 통합관리 솔루션(서드파티 솔루션)을 통해 연동하는 판매자는 `serviceId`를 **별도로 신청하지 않아도 됩니다.**
해당 솔루션이 이미 발급받은 자체 `serviceId`로 그립 API를 호출하며, 판매자는 2번 단계에서 발급한 `accessKey` / `secretKey`를 해당 솔루션에 등록하기만 하면 됩니다.
...
세 가지(입점 계정, `accessKey`/`secretKey`, `serviceId`)가 모두 준비되면 API를 호출할 수 있습니다. 모든 요청에는 아래 인증 헤더가 필요합니다. (쇼핑몰 통합관리 솔루션을 이용하는 경우 아래 헤더 구성은 솔루션이 처리하므로, 판매자는 `accessKey` / `secretKey`만 솔루션에 등록하면 됩니다.)
