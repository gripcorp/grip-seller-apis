### ⚠️ Breaking Change 안내 — 도메인별 순차 적용 완료 (2026.08.21 ~ 09.04)

판매자 OPEN API의 내부 처리 시스템이 개선되었습니다. API의 URL, 요청/응답 필드, 페이징 규칙은 변경되지 않았지만, 오류 응답 방식과 일부 검증 동작이 아래와 같이 달라졌으므로 연동 코드 확인이 필요합니다.

전환은 API 도메인 단위로 점진 적용되었으며, **2026.09.04에 모든 도메인의 전환이 완료되었습니다.**

#### 적용 일정

| 도메인 | 대상 API | 전환 시작 | 전환 완료 |
| --- | --- | --- | --- |
...
| 배송 | [배송](docs/delivery/README.md) | 2026.08.31 | **2026.09.04 (완료)** |

- **모든 도메인의 전환이 완료되어, 아래의 변경 사항이 전체 API에 적용됩니다.** 이제 모든 API의 오류 응답은 항상 변경된 방식(`4xx` 체계)으로 응답됩니다.
