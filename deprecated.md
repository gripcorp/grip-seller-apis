...
- 주문 및 배송 처리 API는 요청을 받고 바로 응답하므로 처리 결과를 대기해야 하는 경우에는 /result 라는 이름의 API를 사용할 수 있습니다. (일부 API만 제공)
...
# 보안 요구사항
- GRIP 판매자센터 API는 소중한 정보를 안전하게 보호하기 위해 암호화 통신 프로토콜인 TLS 를 사용하고 있습니다.
- 보안 강화를 위해 TLS 1.2 이상의 환경에서만 정상적으로 동작합니다. API 요청 시 사용하시는 환경의 TLS 버전을 확인해 주시기 바랍니다.
...
| zeroPriceReasonText       | String                | 0원 상품 사유 상세. 0원 상품 사유 유형이 4인 경우 필수, 나머지 유형은 사용되지 않음. | 최대 50자 |
